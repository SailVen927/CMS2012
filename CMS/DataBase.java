...
public class DataBase {
 ...
 //返回SQL语句结果(finish)
   ...
   public JTable show(){
       ...
       rowData=new Vector<>();
       ResultSet rs=dataBase.getRS("Select cid,cbrand,ctype,color,cprice FROM Car WHERE state=/'未出租/'");//rs存放查询结果
       try {
			while(rs.next()){
   ...
   public JTable adOrderShow(){
       ...
       DataBase dataBase=new DataBase();
       rowData=new Vector<>();
       rs=dataBase.getRS("Select onum,fk_user_name,cid,cbrand,ctype,color,rentday,returnday FROM Car,orders WHERE Car.cid=orders.fk_cid");//rs存放查询结果
       try {
			while(rs.next()){
				Vector tempVector=new Vector();
   ...
   public JTable userOrderShow(){
       ...
       try {
			while(rs.next()){
				Vector tempVector=new Vector();
				tempVector.add(rs.getString(1));//onum
				tempVector.add(rs.getString(2));//cid
				tempVector.add(rs.getString(3));//cBrand
				tempVector.add(rs.getString(4));//ctype
				tempVector.add(rs.getString(5));//color
				tempVector.add(rs.getString(6));//rentday
				rowData.add(tempVector);
			}
		} catch (SQLException e){
 ...
 //验证登录信息
   ...
   public void updateInform(String newInfom,String table,String colname,String condition){
	   try {
		  System.out.println("UPDATE "+table+" SET "+colname+"=\'"+newInfom+"\' WHERE "+condition);
